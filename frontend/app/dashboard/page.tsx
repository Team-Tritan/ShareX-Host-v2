...
const fetchImages = async (apiToken: string, setUploads: (uploads: Upload[]) => void, setLoading: (loading: boolean) => void): Promise<void> => {
  try {
...
const handleDelete = async (apiToken: string, FileName: string, removeUpload: (FileName: string) => void): Promise<void> => {
  try {
...
const Dashboard: React.FC = () => {
  ...
  const totalStorageUsed: number = uploads.reduce((acc, image) => acc + image.Metadata.FileSize, 0);
  const totalFilesUploaded: number = uploads.length;
  const totalViews: number = uploads.reduce((acc, image) => acc + image.Metadata.Views, 0);

  // eslint-disable-next-line react-hooks/rules-of-hooks
  useEffect(() => {
    fetchImages(userStore.apiToken, setUploads, setLoading);
    const intervalId = setInterval(() => fetchImages(userStore.apiToken, setUploads, setLoading), 10000);
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="overflow-hidden rounded-lg bg-[#121114] animate-pulse">
            ...
            {uploads.length === 0 ? (
              <>
                <div className="col-span-full flex items-center justify-center text-gray-400 rounded-lg">
                  ...
                  <div className="col-span-full text-gray-400 rounded-lg">You do not have any uploads.</div>
                </div>
                {Array.from({ length: 6 }).map((_, index) => (
                  <div key={index} className="overflow-hidden rounded-lg bg-[#121114] animate-pulse">
                    <div className="h-48 w-full bg-[#121114]"></div>
              ...
              uploads.map((image: Upload) => (
                <div key={image.Key} className="relative overflow-hidden rounded-lg bg-[#121114] group shadow-2xl shadow-indigo-500/20 transition-all duration-300 hover:shadow-indigo-500/40 hover:scale-105">
                  <img src={`https://s3.tritan.gg/images/${image.FileName}`} alt={image.FileName} className="h-48 w-full object-cover" />
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    <button className="flex items-center rounded bg-pink-500 px-3 py-2 text-sm font-semibold text-white hover:bg-pink-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-colors duration-300" onClick={() => handleDelete(userStore.apiToken, `${image.FileName.split(".").slice(0, -1).join(".")}`, removeUpload)}>
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-sm text-gray-400 mt-2">Uploaded on {new Date(image.Metadata.UploadDate).toLocaleString()}</p>
